...
\begin{document}
	...
	\NI Laplace transform can be performed on partial derivatives, but with one variable at a time ONLY, as long as the variable to be transformed satisfy the condition of $0 \leq(\text{ variable }) < \infty$.
	...
	\subsection*{Solution}	
	From the general form of non-homogeneous partial differential equation with initial condition
	\begin{gather*}
		L_v(x,y) + R_v(x,y) = h(x,y)\sps
		V(x,0) = f(x) \qquad V_y(0,y)=g(y)
	\end{gather*}
	where $\dsp L = \frac{\partial}{\partial x \partial y}, R_v(x,y)$ in the remaining linear term in which contain only first order partial derivatives of $V(x,y)$ with respect to either $x$ or $y$ and $h(x,y)$ is the source term.\sps
	Thus, from the above initial value problem (\ref{p:4_1_1}) ;
	\begin{equation*}
		 L_v(x,y) = \frac{\partial^2 v}{\partial x \partial y}~~;\qquad L(x,y)=e^{-2y}\sin x
	\end{equation*}
	and $Rv(x,y)$ is zero.
	...
	\begin{equation}
		\frac{\partial^2 v}{\partial x \partial y} + \frac{\partial v}{\partial x} + u = 6x^2 y\tag{1}\label{eq:p_3_1}
	\end{equation}
	with initial conditions
	\begin{equation}
		v(x,0)=1,\qquad v(0,y)=y,\qquad v_y(0,y)=0\tag{2}\label{eq:p_3_2}
	\end{equation}
	In the above problem, $\dsp R_v(x,y) = \frac{\partial^2 v}{\partial x \partial y} + v(x,y)$. Use the substitution $\dsp \frac{\partial v}{\partial y}= V(x,y)$ in \refx{p_3_1}, we get
	\begin{equation}
		\frac{\partial V}{\partial x} + \frac{\partial v}{\partial x} + v = 6x^2 y\tag{3}\label{eq:p_3_3}
	\end{equation}
	Taking the Laplace transform of equation \refx{p_3_3} with respect to $x$, we get
	\begin{equation*}
		sV(s,y) - V(0,y) + sv(s,y) - v(0,y) + \Laplace_x\left[v(x,y)\right]\frac{12y}{s^2} 
	\end{equation*}
	\begin{equation}
		V(s,y) = -v(s,y) - \frac{1}{s}\Laplace_x\left[v(x,y)\right]+ \frac{12y}{s^4}\tag{4}\label{eq:p_3_4}
	\end{equation}
	Taking inverse Laplace transform of equation \refx{p_3_4} with respect to $x$, we get 
	\begin{eqnarray*}
		V(x,y) = -v(x,y) - \InverseLx{\frac{1}{s}\Laplace_x\left[v(x,y)\right]} + 2yx^3
	\end{eqnarray*}
	\begin{equation}
		\frac{\partial v(x,y)}{\partial x} = - v(x,y) - \InverseLx{\frac{1}{s}\Laplace_x\left[v(x,y)\right]}+2yx^3\tag{5}\label{eq:p_3_5}
	\end{equation}
	Taking inverse Laplace transform of equation \refx{p_3_5} with respect to $y$, we get
	\begin{equation}
		v(x,y) = 1-\Laplace_y^{-1}\left\{\frac{1}{s}\Laplace_y\left[v(x,y) + \InverseLx{\frac{1}{s}\Laplace_x\left[v(x,y)\right]}\right]\right\} + x^3 y^2\tag{6}\label{eq:p_3_6}
	\end{equation}
	We cannot solve the equation \refx{p_3_6} because our goal $u(x,y)$ is appeared in both sides of equation \refx{p_3_6}. Therefore the equation \refx{p_3_1} cannot be solved by using LSM because of $R_v(x,y) \neq 0$
